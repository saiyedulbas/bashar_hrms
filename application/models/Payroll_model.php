<?php
	...
	public function get_advance_salaries() {
	  return $this->db->get("xin_advance_salaries");
        ...
        // get bonous type > all employee
	public function get_bonous_types() {
	  return $this->db->get("xin_bonous_type");
	}
        public function get_bonous_types_single($id) {

		$sql = 'SELECT * FROM xin_bonous_type WHERE bonous_type_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
		return $query->row();
	}
        public function add_bonous_type($data){
		$this->db->insert('xin_bonous_type', $data);
        ...
        public function updated_bonous_type_payroll($data,$id) {
            $this->db->where('bonous_type_id', $id);
		if( $this->db->update('xin_bonous_type',$data)) {
			return true;
		} else {
			return false;
		}
        }
        public function delete_bonous_type_record($id){
		$this->db->where('bonous_type_id', $id);
		$this->db->delete('xin_bonous_type');

                $this->db->where('bonous_type_id', $id);
		$this->db->delete('xin_employee_bonous');

	}

        public function update_employee_bonous_by_type($data,$bonous_type_id) {
            $this->db->where('bonous_type_id', $bonous_type_id);
            if( $this->db->update('xin_employee_bonous',$data)) {
                    return true;
            } else {
                    return false;
            }
        }
        public function get_employee_bonous(){
            return $this->db->get("xin_employee_bonous");
        }


        public function add_employee_bonous($data){
		$this->db->insert_batch('xin_employee_bonous', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
        ...
        public function get_employee_leave_encashment(){
            return $this->db->get("xin_employee_leave_encashment");
        }
        public function get_employee_leave_encashment_year($emp_id,$month_year){

                $sql = 'SELECT * FROM xin_employee_leave_encashment WHERE employee_id = ? AND month_year = ?';
		$binds = array($emp_id,$month_year);
		$query = $this->db->query($sql, $binds);
		return $query->result();
        }

        public function add_employee_leave_encashment($data){
		$this->db->insert_batch('xin_employee_leave_encashment', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
        ...
        public function get_employee_earn_leave($employee_id){
                $year=date('Y');
                $sql = 'SELECT * FROM xin_employee_earn_leave WHERE employee_id = ? AND year < ? AND status= ? ';
		$binds = array($employee_id,$year,0);
		$query = $this->db->query($sql, $binds);
		return $query->result();
        }
        public function update_employee_leave_encashment($id,$data){
		$this->db->where('id', $id);
		if( $this->db->update('xin_employee_earn_leave',$data)) {
			return true;
		} else {
			return false;
		}
	}
        public function get_active_bonouses($employee_id) {
                $sql = 'SELECT * FROM xin_employee_bonous WHERE employee_id = ? AND status=?';
		$binds = array($employee_id,1);
		$query = $this->db->query($sql, $binds);
		return $query->result();
        }

	public function get_payslip_bonous($payslip_id) {
                $sql = 'SELECT * FROM xin_salary_payslip_bonous WHERE payslip_id = ?';
		$binds = array($payslip_id);
		$query = $this->db->query($sql, $binds);
		return $query->result();
        }
	...
	public function employees_payment_history() {
            $sql = 'SELECT * FROM xin_salary_payslips WHERE status = ?';
		$binds = array(2);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	...
	// new payroll > payslip
	public function all_employees_payment_history($status) {
	  	$sql = 'SELECT * FROM xin_salary_payslips WHERE status= ?';
                $binds = array($status);
		$query = $this->db->query($sql,$binds);
		return $query;
	...
	// new payroll > payslip
	public function all_employees_payment_history_month($salary_month,$salary_end_month,$status) {
	  	$sql = 'SELECT * FROM xin_salary_payslips WHERE salary_month >= ? and salary_month<=? and status= ?';
		$binds = array($salary_month,$salary_end_month,$status);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	// get payslip history > company
	public function get_company_payslip_history($company_id,$status) {
	...
	// get payslip history > company
	public function get_company_payslip_history_month($company_id,$salary_month,$salary_end_month,$status) {

		$sql = 'SELECT * FROM xin_salary_payslips WHERE company_id = ? and salary_month >= ? and salary_month<= ? and status=?';
		$binds = array($company_id,$salary_month,$salary_end_month,$status);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	// get company/location payslips
	public function get_company_location_payslips($company_id,$location_id,$status) {

		$sql = 'SELECT * FROM xin_salary_payslips WHERE company_id = ? and location_id = ? and status= ?';
		$binds = array($company_id,$location_id,$status);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	// get company/location payslips
	public function get_company_location_payslips_month($company_id,$location_id,$salary_month,$salary_end_month,$status) {

		$sql = 'SELECT * FROM xin_salary_payslips WHERE company_id = ? and location_id = ? and salary_month >= ? and salary_month<= ? and status= ?';
		$binds = array($company_id,$location_id,$salary_month,$salary_end_month,$status);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	// get company/location/departments payslips
	public function get_company_location_department_payslips($company_id,$location_id,$department_id,$status) {

		$sql = 'SELECT * FROM xin_salary_payslips WHERE company_id = ? and location_id = ? and department_id = ? and status= ?';
		$binds = array($company_id,$location_id,$department_id,$status);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	// get company/location/departments payslips
	public function get_company_location_department_payslips_month($company_id,$location_id,$department_id,$salary_month,$salary_end_month,$status) {

		$sql = 'SELECT * FROM xin_salary_payslips WHERE company_id = ? and location_id = ? and department_id = ? and salary_month >= ? and salary_month<= ? and status= ?';
		$binds = array($company_id,$location_id,$department_id,$salary_month,$salary_end_month,$status);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	// get company/location/departments payslips
	...
	public function add_salary_payslip_other_payments($data){
        ...
        public function add_bonous_payslip_payments($data) {
            $this->db->insert('xin_salary_payslip_bonous', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function update_payroll_status($data, $id){
        ...
        public function update_salary_loan_deduction($id,$data){
		...
		if( $this->db->update('xin_salary_loan_deductions',$data)) {
         ...
         // Function to Delete selected record from table
	public function delete_payslip_bonous_items($id){
		$this->db->where('payslip_id', $id);
		$this->db->delete('xin_salary_payslip_bonous');
        ...
        public function read_salary_payslip_loans($id) {
        ...
        public function delete_salary_payslip_loans($id){
		$this->db->where('payslip_loan_id', $id);
		$this->db->delete('xin_salary_payslip_loan');
