...
class Chat extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function newpusher(){
		...
		$data['title'] = $this->lang->line('xin_support').' | '.$this->Xin_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('xin_hr_chat_box');
		$data['path_url'] = 'chatbox';
		//$data['all_active_employees'] = $this->Xin_model->all_active_employees();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		...
		$data['subview'] = $this->load->view("admin/pusher/pusher", $data, TRUE);
		$data['subview2'] = $this->load->view("admin/pusher/pusher2", $data, TRUE);
		  ...
		  $("#xin-form").submit(function(e){
			...
			$.ajax({
				...
				success: function(JSON)
				{
					...
					$("#message_content").val("");
					$("input[name="csrf_hrms"]").val(data.csrf_hash);
