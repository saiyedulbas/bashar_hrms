$(document).ready(function() {
   ...
   var xin_table = $('#xin_table').dataTable({
	...
	$('[data-plugin="select_hrm"]').select2({ width:'100%' });
        jQuery.get(base_url+"/get_leave_personal/"+jQuery('#company_id').val(), function(data, status){
        jQuery('#employee_alt').html(data);
     });
	jQuery("#aj_company").change(function(){
		jQuery.get(base_url+"/get_leave_employees/"+jQuery(this).val(), function(data, status){
			jQuery('#employee_ajax').html(data);
		});
                jQuery.get(base_url+"/get_leave_personal/"+jQuery(this).val(), function(data, status){
                jQuery('#employee_alt').html(data);
             });
	});
	//employee_id
