...
class Accounting extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function expense() {
		...
		$data['title'] = $this->lang->line('xin_tax_chalan').' | '.$this->Xin_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('xin_tax_chalan');
		$data['path_url'] = 'tax_chalan';
                $data['all_companies'] = $this->Xin_model->get_companies();
                ...
                $data['chalan_years']=$years;
                $role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('100',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/tax_chalans/tax_chalan", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	...
	public function chalan_list()
     {
		...
		if(!empty($session)){ 
			$this->load->view("admin/tax_chalans/tax_chalan", $data);
		} else {
			redirect('admin/');
		}
		...
		$tax_chalan = $this->Finance_model->get_tax_chalan();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		$data = array();
          foreach($tax_chalan->result() as $r) {
                        $file='';
                        if($r->chalan_file!='' && $r->chalan_file!='no_file'){
                          $url= site_url('admin/download')."?type=tax_chalan&filename=$r->chalan_file";
                          $file ='<a href="'.$url.'">'.$this->lang->line("xin_download").'</a>';
                        }
                        
			$edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-sm btn-outline-secondary waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-tax_chalan_id="'. $r->id . '"><span class="fas fa-pencil-alt"></span></button></span>';
			...
			$combhr = $edit.$delete;
		   	...
		   	$data[] = array(
				...
				$r->chalan_no,
				$r->year,
                                date("F", mktime(0, 0, 0, $r->month, 10)),
				$r->total_amount,
				$r->bank_name,
				$r->submit_date,
                                $file
	  ...
	  $output = array(
		    "draw" => $draw,
                    "recordsTotal" => $tax_chalan->num_rows(),
                    "recordsFiltered" => $tax_chalan->num_rows(),
                    "data" => $data
		);
	...
	public function add_tax_chalan() {
            $session = $this->session->userdata('username');
		if($this->input->post('add_type')=='tax_chalan') {		
		...
		} else if($this->input->post('chalan_no')==='') {
			$Return['error'] = $this->lang->line('xin_error_chalan_no');
		} else if($this->input->post('total_amount')==='') {
			$Return['error'] = $this->lang->line('xin_error_tax_total_amount');
		} else if($this->input->post('bank_name')==='') {
			$Return['error'] = $this->lang->line('xin_tax_error_bank_name');
		} else if($this->input->post('submit_date')==='') {
			$Return['error'] = $this->lang->line('xin_tax_error_submit_date');
		} 
		 if(is_uploaded_file($_FILES['chalan_file']['tmp_name'])) {
			//checking image type
			$allowed =  array('png','jpg','jpeg','pdf','gif','txt','doc','docx','xls','xlsx');
			$filename = $_FILES['chalan_file']['name'];
			$ext = pathinfo($filename, PATHINFO_EXTENSION);
			
			if(in_array($ext,$allowed)){
				$tmp_name = $_FILES["chalan_file"]["tmp_name"];
				$profile = "uploads/tax_chalan/";
				$set_img = base_url()."uploads/tax_chalan/";
				// basename() may prevent filesystem traversal attacks;
				// further validation/sanitation of the filename may be appropriate
				$name = basename($_FILES["chalan_file"]["name"]);
				$newfilename = 'tax_chalan_'.$this->input->post('month').'_'.$this->input->post('year').'.'.$ext;
		...
		$result = $this->Finance_model->add_tax_chalan($data);
		if ($result == TRUE) {			
			$Return['result'] = $this->lang->line('xin_acc_success_tax_chalan_added');
		} else {
        ...
        public function delete_tax_chalan() {
            ...
            if($this->input->post('is_ajax')=='2') {
                    ...
                    $Return['csrf_hash'] = $this->security->get_csrf_hash();
                    $result = $this->Finance_model->delete_tax_chalan($id);
                    if(isset($id)) {
                            $Return['result'] = $this->lang->line('xin_acc_success_chalan_deleted');
                    } else {
        ...
        public function read_tax_chalan()
	{
		$session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		...
		$data['title'] = $this->Xin_model->site_title();
		$id = $this->input->get('tax_chalan_id');
		$result = $this->Finance_model->read_tax_chalan($id);
		...
		$data = array(
                    'tax_chalan_id' => $result[0]->id,
                    'company_id' => $result[0]->company_id,
                    'chalan_no' => $result[0]->chalan_no,
                    'submit_date' => $result[0]->submit_date,
                    'bank_name' => $result[0]->bank_name,
                    'month' => $result[0]->month,
                    'current_year' => $result[0]->year,
                    'total_amount' => $result[0]->total_amount,
                    'chalan_file' => $result[0]->chalan_file,
                    'chalan_years' => $years,
                    'all_companies' => $all_companies,
                    );
		if(!empty($session)){ 
			$this->load->view('admin/tax_chalans/dialog_tax_chalan', $data);
		} else {
			redirect('admin/');
		}
	}
	...
	public function tax_chalan_update() {
                $session = $this->session->userdata('username');
		if($this->input->post('edit_type')=='tax_chalan') {
		...
		if($_FILES['chalan_file']['size'] == 0) {
			 ...
			 $result = $this->Finance_model->update_tax_chalan($data,$id);
		} else {
			if(is_uploaded_file($_FILES['chalan_file']['tmp_name'])) {
				//checking image type
				...
				if(in_array($ext,$allowed)){
					$tmp_name = $_FILES["chalan_file"]["tmp_name"];
					$tax_file = "uploads/tax_chalan/";
					// basename() may prevent filesystem traversal attacks;
					// further validation/sanitation of the filename may be appropriate
					$lname = basename($_FILES["chalan_file"]["name"]);
					$newfilename = 'tax_chalan_'.$this->input->post('month').'_'.$this->input->post('year').'.'.$ext;
					move_uploaded_file($tmp_name, $tax_file.$newfilename);
					$fname = $newfilename;
					 $data = array(
					'company_id' => $this->input->post('company_id'),
                                        'chalan_no' => $this->input->post('chalan_no'),
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_acc_success_tax_updated');
		} else {
			$Return['error'] = $this->lang->line('xin_error');
		}
		$this->output($Return);
		...
		$data['title'] = $this->lang->line('xin_accounting_chart_of_account').' | '.$this->Xin_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('xin_accounting_chart_of_account');
		$data['path_url'] = 'accounting_chart_of_account';
		
		$data['all_expense_types'] = $this->Expense_model->all_expense_types();
		$data['all_income_categories_list'] = $this->Finance_model->all_income_categories_list();
		$data['all_running_project'] = $this->Finance_model->get_all_rinning_project();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('76',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/accounting/chart_of_account_list", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	...
	public function read_payee() {
		...
		$data = array(
				...
				'credit_account' => $result[0]->credit_account,
				'bank_name' => $result[0]->bank_name,
				'routing_no' => $result[0]->routing_no,
				'created_at' => $result[0]->created_at
				);
		if(!empty($session)){ 
			$this->load->view('admin/accounting/dialog_accounting', $data);
	...
	public function bankwise_transactions_list() {
	 ...
	 public function get_all_payee() {
		...
		$length = intval($this->input->get("length"));
         ...
         public function turnover_report() {
		...
		$data['title'] = $this->lang->line('xin_acc_turnover_report').' | '.$this->Xin_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('xin_acc_turnover_report');
		$data['path_url'] = 'accounting_report_turnover';
		$role_resources_ids = $this->Xin_model->user_role_resource();
                $date_past = date("Y", strtotime('-50 year')); 
                $date_future = date("Y");
                for($i=$date_past;$i<=$date_future;$i++){
                    $years[]=$i;
                } 
		...
		if(in_array('86',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/accounting/report_turnover", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
        ...
        // report turnover list
	public function report_turnover_list() {
		...
		$data['title'] = $this->Xin_model->site_title();
		if(!empty($session)){ 
			$this->load->view("admin/accounting/report_turnover", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
		...
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
		$turnover=$this->Employees_model->read_employee_turnover($this->input->get('year'),$this->input->get('month')??0);
		
                $data = array();
                $data[] = array(
                    $turnover['total_employee'],
                    $turnover['new_joined'],
                    $turnover['male_joined'],
                    $turnover['female_joined'],
                    $turnover['leave_company'],
                    $turnover['male_leave'],
                    $turnover['female_leave']
		);
     ...
     public function ebl_report() {
		...
		$data['title'] = $this->lang->line('xin_accounting_ebl_report').' | '.$this->Xin_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('xin_accounting_ebl_report');
		$data['path_url'] = 'accounting_report_ebl';
		$data['all_bank_cash'] = $this->Finance_model->all_bank_cash();
		$data['all_companies'] = $this->Xin_model->get_companies();
		$data['all_expense_types'] = $this->Expense_model->all_expense_types();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('84',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/accounting/report_ebl_connection", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
        ...
        public function report_ebl_list()
     {
                $session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		...
		$data['title'] = $this->Xin_model->site_title();
		if(!empty($session)){ 
			$this->load->view("admin/accounting/report_ebl_connection", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
		...
		$expense = $this->Finance_model->get_ebl_connection_search($this->input->get('from_date'),$this->input->get('to_date'),$this->input->get('type_id'),$this->input->get('company_id'));
		...
		$data = array();
          ...
          foreach($expense->result() as $r) {
		   ...
		   $data[] = array(
                                $acc_type[0]->account_number,
				$session['email'],
				$payee[0]->payee_name,
				$payee[0]->credit_account,
                                $category,
                                $payee[0]->bank_name,
                                $payee[0]->routing_no,
                                $r->amount,
