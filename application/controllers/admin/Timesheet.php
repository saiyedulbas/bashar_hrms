		...
		if(empty($_FILES['file']['name'])) {
			...
			if(in_array($_FILES['file']['type'],$csvMimes)){
				if(is_uploaded_file($_FILES['file']['tmp_name'])){
					...
					// check file size
					if(filesize($_FILES['file']['tmp_name']) > 5120000) {
					...
					while(($line = fgetcsv($csvFile)) !== FALSE ){
                                                $attendance_date = date("Y-m-d", strtotime($line[5])); ;
						$clock_in = $line[9];
						$clock_out = $line[10];
...
//						$total_work = $hours_in .":".$minutes_in;
				...
				fclose($csvFile);
                                $this->Timesheet_model->delete_gurbage_data();
	 ...
	 public function leave_details() {
		...
		$alternate = $this->Xin_model->read_user_info($result[0]->employee_alternate);
                if(!is_null($alternate)){
                    $alt_name=$alternate[0]->first_name. ' '.$alternate[0]->last_name;
                }else{
                    $alt_name = '--';
                }
		if(!is_null($user)){
	...
	// get company > employees
	 public function get_leave_employees() {
		...
		$length = intval($this->input->get("length"));
         ...
         public function get_leave_personal() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view("admin/timesheet/get_leave_employees_personal", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
		...
		foreach($leave->result() as $r) {
			...
			if(!is_null($company)){
				...
				$comp_name = '--';
                        ...
                        $interval = $this->dateDiffInDays($date1, $date2);
	...
	public function add_task() {
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				foreach($this->input->post('assigned_to') as $p_employee) {
					...
					$user_to = $this->Xin_model->read_user_info($p_employee);
                                        $to_full_name = $user_to[0]->first_name.' '.$user_to[0]->last_name;
					//get company info
			...
			<img src="'.$logo.'" title="'.$cinfo[0]->company_name.'"><br>'.str_replace(array("{var site_name}","{var site_url}","{var task_name}","{var assigned_to}", "{var task_assigned_by}"),array($cinfo[0]->company_name,site_url(),$this->input->post('task_name'),$to_full_name,$full_name),htmlspecialchars_decode(stripslashes($template[0]->message))).'</div>';

					hrms_mail($cinfo[0]->email,$cinfo[0]->company_name,$user_to[0]->email,$subject,$message);
				}
			}
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
	...
	public function add_leave() {
		...
		$employee_alternate = $this->input->post('employee_alternate');
		...
		/*
		* Logic implemented for maintaining casual leave maximum 3 days.
		* The leave type one is the current DB value for leave type
		* and is changable then need to update this logic manually
		*/
		if($this->input->post('leave_type')==="1" && $no_of_days>3){
			$Return['error'] = $this->lang->line('xin_hr_cant_appply_morethan').' 3 '.$this->lang->line('xin_day');
		}
		if($Return['error']!=''){
       		$this->output($Return);
    	}
		...
		if($this->input->post('start_date')!=''){
			...
			//$user_info_all = $this->Employees_model->read_employee_information($this->input->post('employee_id'));
                    if($this->input->post('leave_type')!=-99){
			...
			if($leave_remaining_total < 0.4){
				$Return['error'] = $this->lang->line('xin_leave_limit_msg').' '.$this->lang->line('xin_hrms_leave_quota_completed') .$type_name;
			}
                    }else{
                        ...
                        $earn_leave = $this->Timesheet_model->employee_earn_leaves($session['user_id']);
                        $earn_leave_remaining=0;
                        if($earn_leave->leave_balance){
                            $earn_leave_remaining=$earn_leave->leave_balance-$earn_leave->leave_taken;
                        }
                        if($days_between>$earn_leave_remaining){

                            $Return['error'] = $this->lang->line('xin_leave_limit_msg').' '.$this->lang->line('xin_hrms_leave_quota_completed') .'Earn Leave';
                        }
                    }
			/*$remaining_leave = count_leaves_info($this->input->post('leave_type'),$this->input->post('employee_id'));
			$type = $this->Timesheet_model->read_leave_type_information($this->input->post('leave_type'));
		...
		'remarks' => $qt_remarks,
		'employee_alternate' => $employee_alternate,
		...
		if ($result == TRUE) {
			...
			$setting = $this->Xin_model->read_setting_info(1);
				...
				$reports_to = $this->Xin_model->read_user_info($user_info[0]->reports_to);
                                $emailEmployees = $this->Employees_model->fetch_all_company_employees_by_role(100, 0, $user_info[0]->company_id, 6);
                                $to_email=array();
                                if(!empty($emailEmployees)){
                                foreach ($emailEmployees as $emailRecipient) {
                                    $to_email[]=$emailRecipient->email;
                                    //hrms_mail($cinfo1[0]->email,$cinfo[0]->name,$emailRecipient->email,$subject,$message);
                                    //hrms_mail($cinfo[0]->email, "Ulkasemi HRMS", $emailRecipient->email, $subject, $message);
                                }
                                }
                                if($reports_to){
                                   array_push($to_email, $reports_to[0]->email);
                                }
				...
				if(!empty($to_email)){
					hrms_mail($cinfo[0]->email,$full_name,implode(', ', $to_email),$subject,$message);
				} else {
					hrms_mail($cinfo[0]->email,$full_name,$cinfo[0]->email,$subject,$message);
				}
	...
	public function update_leave_status() {
		...
		'remarks' => $qt_remarks
                ...
                $start_date = strtotime($timesheet[0]->from_date);
                $end_date = strtotime($timesheet[0]->to_date);
                $days_between = (($end_date - $start_date)/60/60/24)+1;

		$result = $this->Timesheet_model->update_leave_record($data,$id);
                $earn_leave = $this->Timesheet_model->employee_curent_earn_leaves($timesheet[0]->employee_id);

                $data1 = array(
		'leave_taken' => $days_between+$earn_leave->leave_taken
		);

                $this->Timesheet_model->update_earn_leave_record($earn_leave->id,$data1);
		...
		if($setting[0]->enable_email_notification == 'yes') {
			...
			<img src="'.$logo.'" title="'.$cinfo[0]->company_name.'"><br>'.str_replace(array("{var site_name}","{var site_url}","{var leave_start_date}","{var leave_end_date}"),array($cinfo[0]->company_name,site_url(),$from_date,$to_date),htmlspecialchars_decode(stripslashes($template[0]->message))).'</div>';
				...
				hrms_mail($cinfo[0]->email,$cinfo[0]->company_name,implode(', ', $to_email),$subject,$message);
			...
			//if($system[0]->system_ip_restriction == 'yes'){
				$sys_arr = explode(',',$system[0]->system_ip_address);
					...
					if($clock_state=='clock_in') {
						...
						$work_time = $this->Timesheet_model->get_all_work_time();
						...
						$total_work = $hours_in .":".$minutes_in;
                                                ...
                                                if($work_time){
                                                    ...
                                                    $total_works[]=$work_time->total_work;
                                                    $total_work=$this->CalculateTime($total_works);
                                                }
