$(document).ready(function(){
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		}else if(field_tpe == 'professional_training'){
			var field_add = '&data=eprofessional_training_info&type=eprofessional_training_info&';
		}
		var modal = $(this);
		$.ajax({
 ...
 //update additional data
	$("#update_additional_details").submit(function(e){
	var fd = new FormData(this);
	var obj = $(this), action = obj.attr('name');
	fd.append("is_ajax", 1);
	fd.append("type", 'additional_info');
	fd.append("data", 'additional_info');
	fd.append("form", action);
	e.preventDefault();
	$('.save').prop('disabled', true);
	$.ajax({
  ...
  //update extra curriculam data
	$("#update_extra_curriculams").submit(function(e){
	var fd = new FormData(this);
	var obj = $(this), action = obj.attr('name');
	fd.append("is_ajax", 1);
	fd.append("type", 'extra_curriculam');
	fd.append("data", 'extra_curriculam');
	fd.append("form", action);
	e.preventDefault();
	$('.save').prop('disabled', true);
	$.ajax({
 ...
 //end professional training info
 //update additional data
 jQuery("#professional_training").submit(function(e){
	/*Form Submit*/
	e.preventDefault();
		...
		jQuery.ajax({
			type: "POST",
			url: e.target.action,
			data: obj.serialize()+"&is_ajax=13&data=professional_training&type=professional_training&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					Ladda.stopAll();
					...
					xin_professional_training.api().ajax.reload(function(){
						Ladda.stopAll();
						toastr.success(JSON.result);
						$('input[name="csrf_hrms"]').val(JSON.csrf_hash);
					}, true);
					$('.icon-spinner3').hide();
					jQuery('#professional_training')[0].reset(); // To reset form fields
					jQuery('.save').prop('disabled', false);
				}
			}
		});
 ...
 //end professional training info
	...
	jQuery("#pre_city").change(function(){
		jQuery.get(base_url+"/get_edistricts/"+jQuery(this).val(), function(data, status){
			jQuery('#edistrict').html(data);
		});

	});
        jQuery("#per_city").change(function(){
		jQuery.get(base_url+"/get_pdistricts/"+jQuery(this).val(), function(data, status){
			jQuery('#eper_district').html(data);
		});
    ...
    // On page load
	var xin_professional_training = $('#xin_professional_training').dataTable({
        "bDestroy": true,
		"ajax": {
            url : site_url+"employees/professional_training/"+$('#user_id').val(),
            type : 'GET'
        },
		"fnDrawCallback": function(settings){
		$('[data-toggle="tooltip"]').tooltip();
	...
	// On page load > xin_hrms_table

	$('.xin_hrms_table').DataTable();
	/* Add contact info */
   ...
   /* Delete data */
	...
	if(tk_type == 'contact'){
		...
		var tb_name = 'xin_table_'+tk_type;
	} else if(tk_type == 'xin_professional_training'){
		var field_add = '&is_ajax=30&data=delete_record&type=delete_professional_training&';
		var tb_name = tk_type;
	}
...
$(document).ready(function(){
 ...
 function printDiv(divName) {
   ...
   $("#salary_statement").click(function(){
		var emp_id = $('#emp_id').val();
		var year = $('#year').val();
		$.ajax({
			url : site_url+"employees/salary_statement",
			type: "get",
                        data: {emp_id:emp_id,year:year},
			success: function (data) {
                            console.log(data);
                            $('#salary_sheet_option').html(data);
			}
		});
	});

    $("#salary_statement_range").click(function(){
		var emp_id = $('#emp_id').val();
		var start_date = $('#start_date').val();
		var end_date = $('#end_date').val();
		$.ajax({
			url : site_url+"employees/salary_statement_range",
			type: "get",
                        data: {emp_id:emp_id,start_date:start_date,end_date:end_date},
			success: function (data) {
                            console.log(data);
                            $('#salary_sheet_option').html(data);
			}
