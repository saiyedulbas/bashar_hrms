...
class Settings extends MY_Controller {
	...
	public function constants()
	 ...
	 public function create_database_backup()
     {
		...
		if($this->input->post('type')==='backup') {
			...
			$name = 'hrms_backup_'.date('d-m-Y').'_'.time(); // name sql backup

			$newImport = new Backup_hrms($hostname,$database,$username,$password);
			$newImport->backup();
	...
	public function company_info() {
		...
		if($this->input->post('type')=='earn_leave') {
		$session = $this->session->userdata('username');
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		...
		if($this->input->post('year')==='') {
       		 $Return['error'] = $this->lang->line('xin_year_error');
		}
		if($Return['error']!=''){
                    $this->output($Return);
                }
                $all_employees = $this->Xin_model->all_employees_active();
                foreach ($all_employees as $employee) {

                    $leave_for=$this->input->post('year')."-12-31";
                    $joining_date=$employee->date_of_joining;
                    $diff = strtotime($leave_for)-strtotime($joining_date);
                    $years = floor($diff / (365*60*60*24));

                    if($years>=2){
                        $current_year_leave=$this->Xin_model->getemployee_current_year_earn_leave($employee->user_id,$this->input->post('year'));
                        $last_year_leave=$this->Xin_model->getemployee_last_earn_leave($employee->user_id);
                        $carry_forowered=0;
                        if($last_year_leave){
                        $available_leave=$last_year_leave->leave_balance-$last_year_leave->leave_taken;
                        if($last_year_leave->leave_encashment==0){
                            if($available_leave>7){
                                $carry_forowered=7;
                            }else{
                                $carry_forowered=$available_leave;
                            }
                        }
                        }
                        if(!$current_year_leave){
                            //return $current_year_leave;
                            $leave_balance=0;
                            if($years==1){
                                $leave_balance=8+$carry_forowered;
                            }
                            if($years==2){
                                $leave_balance=9+$carry_forowered;
                            }
                            if($years==3){
                                $leave_balance=10+$carry_forowered;
                            }
                            if($years==4){
                                $leave_balance=11+$carry_forowered;
                            }
                            if($years==5){
                                $leave_balance=12+$carry_forowered;
                            }
                            if($years==6){
                                $leave_balance=13+$carry_forowered;
                            }
                            if($years==7){
                                $leave_balance=14+$carry_forowered;
                            }
                            if($years>=8){
                                $leave_balance=15+$carry_forowered;
                            }
                            ...
                            $result = $this->Xin_model->insert_earn_leave($data);
                            if ($result == TRUE) {
                                $Return['result'] = $this->lang->line('xin_success_company_info_updated');
                            } else {
                                $Return['error'] = $this->lang->line('xin_error_msg');
	...
	public function layout_skin_info() {
		...
		if($this->input->get('type')=='hrms_layout_info') {
		...
		$data = array(
		'fixed_header' => $this->input->get('fixed_layout_hrms'),
		'boxed_wrapper' => $this->input->get('boxed_layout_hrms'),
		'compact_sidebar' => $this->input->get('sidebar_layout_hrms')
		);
