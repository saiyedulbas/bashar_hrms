$(document).ready(function () {
    ...
    var xin_table = $('#xin_table').dataTable({
        "bDestroy": true,
        "ajax": {
            url: site_url + "payroll/employee_leave_encashment_list",
            type: 'GET'
        },
    ...
    jQuery("#aj_company").change(function(){
		...
		jQuery.get(base_url+"/get_employees/"+c_id, function(data, status){
    ...
    $("#xin-form").submit(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: e.target.action,
            data: obj.serialize() + "&is_ajax=1&add_type=employee_leave_encashment&form=" + action,
            cache: false,
            success: function (JSON) {
                if (JSON.error != '') {
                    toastr.error(JSON.error);
