     ...
     <?php $j=1;foreach($policy->result() as $r) { ?>
      <div id="policy_<?php echo $r->policy_id;?>" class="card animated fadeIn mb-3 tab-pane step-content" <?php if($j==1):?>style="display: block;"<?php else:?>style="display: none;"<?php endif;?>>
        ...
        <h4 class="media align-items-center my-3">
          ...
          <p><?php echo html_entity_decode($r->description);?></p>
          
        <h4><?php echo $this->lang->line('xin_attachment');?></h4>
            <div style="display: table-cell;"><?php if( $r->attachment!='' && $r->attachment!='no file') {?>
              <img src="<?php echo base_url().'uploads/company_policy/'. $r->attachment;?>" width="70px" id="u_file">&nbsp; <a href="<?php echo site_url()?>admin/download?type=company_policy&filename=<?php echo  $r->attachment;?>"><?php echo $this->lang->line('xin_download');?></a>
              <?php } ?></div>
