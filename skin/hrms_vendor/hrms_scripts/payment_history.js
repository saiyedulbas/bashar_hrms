	...
	//var date_format = $('#date_format').val();
		//$('#att_date').html(date_format);
		 var xin_table2 = $('#xin_table').dataTable({
			"bDestroy": true,
			"ajax": {
				url : site_url+"payroll/payment_history_list/?ihr=true&company_id="+$('#aj_company').val()+"&location_id="+$('#aj_location_id').val()+"&department_id="+$('#aj_subdepartments').val()+"&salary_month="+$('#salary_month').val()+"&salary_end_month="+$('#salary_end_month').val()+"&status="+$('#status').val(),
				type : 'GET'
			},
                        dom: 'lBfrtip',
		"buttons": [{
