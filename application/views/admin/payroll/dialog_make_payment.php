<?php
$CI =& get_instance();
$CI->load->library('taxcalculatorlib');
...
$bonouses=$this->Payroll_model->get_active_bonouses($user_id);
if(!empty($bonouses)){
...
$salaryBreakDown = $CI->taxcalculatorlib->salaryBreakDown($basic_salary,$bonous);

$taxableIncome = $CI->taxcalculatorlib->salaryTaxableIncome($salaryBreakDown);
$finalSalaryTax = $CI->taxcalculatorlib->salaryTaxCalculation($taxableIncome, $gender);
// all other payment
...
// 6: leave_encashment
$leave_encashments = $this->Payroll_model->get_employee_leave_encashment_year($user_id,$this->input->get('pay_date'));
...
// add amount
$add_salary = $allowance_amount + $basic_salary + $overtime_amount + $all_other_payment + $commissions_amount + $saudi_gosi+$total_bonous_amount+$total_leave_encashment_amount;
// add amount
...
$net_salary = $net_salary - ($advance_amount+$finalSalaryTax['monthlyTDS']);
?>
...
<?php echo form_open('admin/payroll/add_pay_monthly/', $attributes, $hidden);?>
   <div class="row">
    ...
    <div class="row">
        ...
        <div class="col-md-4">
        <div class="form-group">
          <label for="total_bonous">Total Bonous</label>
          <input type="text" name="total_bonous" class="form-control" value="<?php echo $total_bonous_amount;?>" readonly="readonly">
        </div>
      </div>
        <div class="col-md-4">
        <div class="form-group">
          <label for="total_leave_encashment">Leave Encashment</label>
          <input type="text" name="total_leave_encashment" class="form-control" value="<?php echo $total_leave_encashment_amount;?>" readonly="readonly">
        </div>
      </div>
        <div class="col-md-4">
        <div class="form-group">
          <label for="tax_deduction">Tax Deduction</label>
          <input type="text" name="tax_deduction" class="form-control" value="<?php echo $finalSalaryTax['monthlyTDS'];?>" readonly="readonly">
        </div>
      </div>
    ...
    <div class="form-actions box-footer"> <?php echo form_button(array('name' => 'hrms_form', 'type' => 'submit', 'class' => $this->Xin_model->form_button_class(), 'content' => '<i class="fas fa-check-square"></i> '.$this->lang->line('xin_pay'))); ?> </div>
  <?php echo form_close(); ?>
</div>
</div>
