...
class Dashboard extends MY_Controller {
	...
	public function hrms_roles()
	{
		...
		$someArray = array();
		$j=0;
		foreach(hrms_roles() as $hr_roles) {
	...
	// get hrms roles > chart
	public function hrms_office_shifts()
	{
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('chart_data'=>'', 'c_name'=>'', 'd_rows'=>'','c_color'=>'');
		$c_name = array();
		...
		$someArray = array();
		$j=0;
		foreach(hrms_office_shift() as $hr_office_shift) {
	...
	// get payroll | salary
	public function hrms_user_payroll()
	{
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('chart_data'=>'', 'c_name'=>'', 'c_am'=>'');
		$c_name = array();
		...
		{
		   $months = date("Y-m", strtotime( date( 'Y-m-01' )." -$i months"));
		   $amount = ihrms_user_payroll($months,$session['user_id']);
		   $payroll_amount = $amount;
		   $c_name[] = $months;
			$someArray[] = array(
				'label'   => $months,
	...
	// get expense deposit
	public function hrms_expense_deposit()
	{
		/* Define return | here result is used to return user data and error for error message */
	...
	// overtime request
	public function hrms_overtime_request()
	{
		/* Define return | here result is used to return user data and error for error message */
	...
	// hrms clients leads
	public function hrms_clients_leads()
	{
		/* Define return | here result is used to return user data and error for error message */
	...
	public function notifications()
	{
		...
		//$this->load->view('admin/settings/hrms_notifications', $data);
		$data['subview'] = $this->load->view("admin/settings/hrms_notifications", $data, TRUE);
		$this->load->view('admin/layout/layout_main', $data); //page load
