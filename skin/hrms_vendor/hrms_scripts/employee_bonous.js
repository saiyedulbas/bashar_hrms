$(document).ready(function () {
    ...
    var xin_table = $('#xin_table').dataTable({
        "bDestroy": true,
        "ajax": {
            url: site_url + "payroll/employee_bonous_list",
            type: 'GET'
        },
...
//    jQuery("#amount_option").change(function(){
//		var amount_option = jQuery(this).val();
//		if(amount_option==0){
...
//	});
    ...
    $("#xin-form").submit(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: e.target.action,
            data: obj.serialize() + "&is_ajax=1&add_type=employee_bonous&form=" + action,
            cache: false,
            success: function (JSON) {
                if (JSON.error != '') {
                    toastr.error(JSON.error);
