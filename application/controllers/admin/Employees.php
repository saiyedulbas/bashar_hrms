...
class Employees extends MY_Controller {
	...
	public function hr() {
		...
		if($this->input->post("hrms_directory")==1){
	 ...
	 public function employees_cards_list()
     {
	  ...
	  public function detail() {
		...
		//$data['path_url'] = 'employees_detail';
                $date_past = date("Y", strtotime('-100 year'));
                $date_future = date("Y", strtotime('10 year'));
                for($i=$date_past;$i<=$date_future;$i++){
                    $years[]=$i;
                }
		$data = array(
			...
			'e_tin' => $result[0]->e_tin,
			'department_id' => $result[0]->department_id,
			'sub_department_id' => $result[0]->sub_department_id,
			'designation_id' => $result[0]->designation_id,
			'daily_food_allowance' => $result[0]->daily_food_allowance,
	...
	public function dialog_contact() {
	 ...
	 public function get_company_elocations() {
		...
		$length = intval($this->input->get("length"));
         ...
         public function get_edistricts() {
		...
		if(is_numeric($keywords[0])) {
			...
			$data = array(
				'division_id' => $id,
				'name' => 'pre_district',
				'district_id' => 'pre_district',
                                'label' => $this->lang->line('xin_e_pre_district')
				);
			$session = $this->session->userdata('username');
			if(!empty($session)){
				$data = $this->security->xss_clean($data);
				$this->load->view("admin/employees/get_districts", $data);
			} else {
				redirect('admin/');
			}
		}
		...
		$length = intval($this->input->get("length"));
          ...
          public function get_pdistricts() {
		...
		if(is_numeric($keywords[0])) {
			...
			$data = array(
				'division_id' => $id,
                                'name' => 'per_district',
				'district_id' => 'per_district',
				'label' => $this->lang->line('xin_e_per_district')
				);
			$session = $this->session->userdata('username');
			if(!empty($session)){
				$data = $this->security->xss_clean($data);
		...
		if(is_numeric($keywords[0])) {
			...
			$data = array(
				...
				'upazila_id' => 'per_police_station',
                                'label'=>$this->lang->line('xin_e_per_police_station')
				);
			$session = $this->session->userdata('username');
			if(!empty($session)){
				$data = $this->security->xss_clean($data);
				$this->load->view("admin/employees/get_upazilas", $data);
			} else {
				redirect('admin/');
			}
		}
	...
	public function add_employee() {
		...
		$module_attributes = $this->Custom_fields_model->all_hrms_module_attributes();
			 ...
			 foreach($module_attributes as $mattribute) {
				 if($mattribute->validation == 1){
					 if($i!=1) {
						...
						$Return['error'] = $this->lang->line('xin_hrms_custom_field_the').' '.$mattribute->attribute_label.' '.$this->lang->line('xin_hrms_custom_field_is_required');
					}
				 }
			 }
			 if($Return['error']!=''){
		...
		if ($iresult) {
			...
			if($count_module_attributes > 0){
				foreach($module_attributes as $mattribute) {
					...
					/*$attr_orig_value = $this->Custom_fields_model->read_hrms_module_attributes_values($result,$mattribute->custom_field_id);
			...
			<img src="'.$logo.'" title="'.$cinfo[0]->company_name.'"><br>'.str_replace(array("{var site_name}","{var site_url}","{var username}","{var employee_id}","{var employee_name}","{var email}","{var password}"),array($cinfo[0]->company_name,site_url(),$this->input->post('username'),$this->input->post('employee_id'),$full_name,$this->input->post('email'),$this->input->post('password')),htmlspecialchars_decode(stripslashes($template[0]->message))).'</div>';
				...
				hrms_mail($cinfo[0]->email,$cinfo[0]->company_name,$this->input->post('email'),$subject,$message);
			}
	...
	public function basic_info() {
		...
		'daily_food_allowance' => $this->input->post('daily_food_allowance'),
	...
	//update additional details of employee
        // Validate and update info in database // basic info
	public function update_additional_details() {

		if($this->input->post('type')=='additional_info') {
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		...
		/* Server side PHP input validation */
		if($this->input->post('other_name')=='') {
                    $Return['error'] = $this->lang->line('xin_employee_error_other_name');
		}
		if($Return['error']!=''){
       		$this->output($Return);
                }
		...
		'pre_house_no' => $this->input->post('pre_house_no'),
		'pre_street1' => $this->input->post('pre_street1'),
		'pre_street2' => $this->input->post('pre_street2'),
		'pre_city' => $this->input->post('pre_city'),
		'pre_district' => $this->input->post('pre_district'),
		'pre_country' => $this->input->post('pre_country'),
		'pre_tel' => $this->input->post('pre_tel'),
		'pre_mobile' => $this->input->post('pre_mobile'),
		'per_house_no' => $this->input->post('per_house_no'),
		'per_police_station' => $this->input->post('per_police_station'),
		...
		'per_mobile' => $this->input->post('per_mobile'),
		'employee_id' => $this->input->post('employee_id'),
		);
		$employee_id = $this->input->post('employee_id');
                $additional_details=$this->Employees_model->read_employee_additional($employee_id);
                if($additional_details){
                    $result = $this->Employees_model->update_additional_info($data,$employee_id);
                }else{
                    $result = $this->Employees_model->add_additional($data);
                }

		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_employee_additional_info_updated');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
        ...
        //update extera curriculam
        public function update_extra_curriculams() {
              $session = $this->session->userdata('username');
		if($this->input->post('type')=='extra_curriculam') {
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		...
		$employee_id = $this->input->post('employee_id');
                $extra_curriculam=$this->Employees_model->read_employee_extra_curriculam($employee_id);
                if($extra_curriculam){
                    $result = $this->Employees_model->update_extra_curriculam($data,$employee_id);
                }else{
                    $result = $this->Employees_model->add_extra_curriculam($data);
                }

		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_employee_extra_curriculam_updated');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
        ...
        // employee professional training- listing
	public function professional_training() {
		...
		$id = $this->uri->segment(4);
		$professional_trainings = $this->Employees_model->set_employee_professional_training($id);

		$data = array();

        foreach($professional_trainings->result() as $r) {
            $training_category=$this->Location_model->get_training_ategory_info($r->training_category);

		$data[] = array(
			'<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-sm btn-outline-secondary waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-data" data-field_id="'. $r->id . '" data-field_type="professional_training"><i class="fas fa-pencil-alt"></i></button></span><span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->id . '" data-token_type="xin_professional_training"><i class="fas fa-trash-restore"></i></button></span>',
			$training_category[0]->title,
			$r->title,
			$r->institute,
			$r->time_period,
			$r->year
	  ...
	  $output = array(
		   "draw" => $draw,
			 "recordsTotal" => $professional_trainings->num_rows(),
			 "recordsFiltered" => $professional_trainings->num_rows(),
			 "data" => $data
		);
	  echo json_encode($output);
	  exit();
     }
     	public function eprofessional_training_info() {

		if($this->input->post('type')=='eprofessional_training_info') {
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();

		/* Server side PHP input validation */
		if($this->input->post('training_category')=='') {
                    $Return['error'] = $this->lang->line('xin_professional_training_category_error');
                }elseif($this->input->post('title')==''){
                    $Return['error'] = $this->lang->line('xin_professional_training_title_error');
                }
                elseif($this->input->post('institute')==''){
                    $Return['error'] = $this->lang->line('xin_professional_training_institute_error');
                }
                elseif($this->input->post('time_period')==''){
                    $Return['error'] = $this->lang->line('xin_professional_training_time_period_error');
                }
                elseif($this->input->post('year')==''){
                    $Return['error'] = $this->lang->line('xin_professional_training_year_error');
                }
		if($Return['error']!=''){
       		$this->output($Return);
    	}
		...
		$result = $this->Employees_model->professional_training_info_update($data,$e_field_id);
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_employee_professional_training_updated');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
        ...
        // Validate and update info in database // basic info
	public function add_professional_training() {
              $session = $this->session->userdata('username');
		if($this->input->post('type')=='professional_training') {
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		...
		'added_by' => $session['user_id'],
                ...
                $result = $this->Employees_model->add_professional_training($data);


		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_record_added');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
        ...
        // delete professional training record
	public function delete_xin_professional_training() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$id = $this->uri->segment(4);
			$result = $this->Employees_model->delete_professional_training($id);
			if(isset($id)) {
				$Return['result'] = $this->lang->line('xin_professional_training_deleted');
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
	...
	public function update_statutory_deductions_info() {
		...
		if($this->input->post('title')==='') {
			...
			$Return['error'] = 'Ordering is required';
		}
			...
			if(in_array($_FILES['file']['type'],$csvMimes)){
				if(is_uploaded_file($_FILES['file']['tmp_name'])){
					...
					while(($line = fgetcsv($csvFile)) !== FALSE){
						...
						'email' => $line[5],
						'pincode' => $line[6],
						'password' => password_hash($line[7], PASSWORD_BCRYPT, $options),
						'blood_group' => $line[8],
						'date_of_birth' => $line[9],
						'n_id' => $line[10],
                                                'contact_no' => $line[11],
                                                'gender' => $line[12],
                                                'nationality_id' => $line[13],
                                                'ethnicity_type' => $line[14],
                                                'date_of_joining' => $line[15],
                                                'company_id' => $line[16],
                                                'location_id' => $line[17],
                                                'department_id' => $line[18],
                                                'designation_id' => $line[19],
                                                'user_role_id' => $line[20],
                                                'marital_status' => $line[33],
						'created_at' => date('Y-m-d h:i:s')
						);
					$result = $this->Employees_model->add($data);
                                        $additional=array(
                                            ...
                                            'father_name' => $line[23],
                                            'mother_name' => $line[24],
                                            'pre_house_no' => $line[25],
                                            'pre_street1' => $line[26],
                                            'pre_street2' => $line[27],
                                            'pre_tel' => $line[28],
                                            'pre_mobile' => $line[29],
                                            'per_house_no' => $line[30],
                                            'per_tel' => $line[31],
                                            'per_mobile' => $line[32],
                                        ...
                                        $result1 = $this->Employees_model->add_additional($additional);

                                        $contacts=array(
                                            'employee_id' => $result,
                                            'contact_name' => $line[36],
                                            'mobile_phone' => $line[37],
                                            'address_1' => $line[38],
                                            'relation' => $line[39],
                                            'work_email' => $line[40],
                                        );
                                        $result2 = $this->Employees_model->contact_info_add($contacts);
				}
	...
	public function dialog_security_level() {
        ...
        public function dialog_professional_training() {
		...
		$data['title'] = $this->Xin_model->site_title();
		$id = $this->input->get('field_id');
		$result = $this->Employees_model->read_professional_training_info($id);

		$data = array(
                    'id' => $result[0]->id,
                    'employee_id' => $result[0]->employee_id,
                    'training_category' => $result[0]->training_category,
                    'title' => $result[0]->title,
		...
		$data['title'] = $this->Xin_model->site_title();
                $user = $this->Xin_model->read_user_info($this->input->get('emp_id'));
                ...
                $start_month=7;
                $end_month=6;
                $start_session=($year-1).'-'.'07';
                $end_session=$year.'-'.'06';
                $start_year=$year-1;
                $period=date( "d M Y", strtotime('01'.'-'.$start_month.'-'.$start_year)).' - '.date( "d M Y", strtotime('30'.'-'.$end_month.'-'.$year));
                $payslips=$this->Employees_model->get_payslip_by_year_month($this->input->get('emp_id'),$start_session,$end_session);

                $tax_chalans=$this->Employees_model->get_tax_chalan_by_month($start_month,$end_month,$year);
                $last_salary=$this->Employees_model->get_payslip_last_salary($this->input->get('emp_id'));
                    ...
                    $medicalAllowance=0;
                    $total_bonous=0;
                    $total_leave_encashment=0;
                    $net_salary=0;
                    $tax_deduction=0;
                    $total_allowances=0;
                    $basic_salary=0;
                foreach($payslips as $payslip){
                    $salary[]=$payslip->net_payable_salary;
                    $payslip_ids[]=$payslip->payslip_id;
                    $basicSalary+=$payslip->basicSalary;
                    $houseRent+=$payslip->houseRent;
                    $totalConveyence+=$payslip->totalConveyence;
                    $medicalAllowance+=floatval($payslip->medicalAllowance);
                    $total_bonous+=floatval($payslip->total_bonous);
                    $total_leave_encashment+=floatval($payslip->total_leave_encashment);
                    $net_salary+=floatval($payslip->net_salary);
                    $tax_deduction+=floatval($payslip->tax_deduction);
                    $total_allowances+=floatval($payslip->total_allowances);
                    $basic_salary+=floatval($payslip->basic_salary);
                }


                $additional_details=$this->Employees_model->read_employee_additional($user[0]->user_id);

                $loans=array();
                $loans=$this->Employees_model->get_employees_loan($payslip_ids);
                $saturary_deductions=array();
                $saturary_deductions=$this->Employees_model->get_saturary_deductions($payslip_ids);
                if($additional_details){

                $city = $this->Location_model->get_division($additional_details->pre_city);
		$district=$this->Location_model->get_district($additional_details->pre_district);
                }
		$data = array(
                    'user' => $user[0],
                    'company' => $company[0],
                    'payslips' => $payslips,
                    'tax_chalans' => $tax_chalans,
                    'period' => $period,
                    'loans' => $loans,
                    'saturary_deductions' => $saturary_deductions,
                    'city' => $city->name??null,
                    'district' => $district->name??null,
                    'additional_details' => $additional_details,
                    'net_salary'=>$net_salary,
                    'tax_deduction'=>$tax_deduction,
                    'total_bonous'=>$total_bonous,
                    'total_leave_encashment'=>$total_leave_encashment,
		...
		if(!empty($session)){
		return	$this->load->view('admin/employees/dialog_salary_sheet', $data);
		} else {
			redirect('admin/');
		}
	}
         public function salary_statement_range() {
		$this->load->library("taxcalculatorlib");
		$session = $this->session->userdata('username');
		if(empty($session)){
			redirect('admin/');
		...
		$data['title'] = $this->Xin_model->site_title();
                ...
                $end_month=date("m", $start_date);


//                $start_session=date(strtotime('Y-m',$this->input->get('start_date')));
//                $end_session=date(strtotime('Y-m',$this->input->get('end_date')));
                $start_session=$start_year.'-'.$start_month;
                $end_session=$end_year.'-'.$end_month;
                $period=date( "d M Y", strtotime($this->input->get('start_date'))).' - '.date( "d M Y", strtotime($this->input->get('end_date')));
                $payslips=$this->Employees_model->get_payslip_by_year_month($this->input->get('emp_id'),$start_session,$end_session);
                ...
                foreach($payslips as $payslip){
                    $salary[]=$payslip->net_payable_salary;
                    $payslip_ids[]=$payslip->payslip_id;
                    $basicSalary+=floatval($payslip->basicSalary);
                    $houseRent+=floatval($payslip->houseRent);
                    $totalConveyence+=floatval($payslip->totalConveyence);
                    $medicalAllowance+=floatval($payslip->medicalAllowance);
                    $total_bonous+=floatval($payslip->total_bonous);
                    $net_salary+=floatval($payslip->net_salary);
                    $tax_deduction+=floatval($payslip->tax_deduction);
		...
		if(!empty($session)){
		return	$this->load->view('admin/employees/dialog_salary_sheet_range', $data);
		} else {
			redirect('admin/');
		}
	}
