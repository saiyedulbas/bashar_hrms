   ...
   /*Function to set JSON output*/
    ...
    public function index()
    {
        ...
        if (in_array('110', $role_resources_ids) || in_array('111', $role_resources_ids) || in_array('112', $role_resources_ids) || in_array('113', $role_resources_ids) || in_array('114', $role_resources_ids) || in_array('115', $role_resources_ids) || in_array('116', $role_resources_ids) || in_array('117', $role_resources_ids) || in_array('409', $role_resources_ids) || in_array('83', $role_resources_ids) || in_array('84', $role_resources_ids) || in_array('85', $role_resources_ids) || in_array('86', $role_resources_ids)) {
            $data['subview'] = $this->load->view("admin/layout/hrms_reports", $data, true);
    ...
    public function role_employees_list()
    {
        ...
        foreach ($employee->result() as $r) {
            ...
            // get status
            if ($r->is_active==0): $status = $this->lang->line('xin_employees_inactive'); elseif ($r->is_active==1): $status = $this->lang->line('xin_employees_active');
            endif;
            // user role
    ...
    public function report_employees_list()
    {
        ...
        foreach ($employee->result() as $r) {
            ...
            // Working duraiton
            $date1 = new DateTime(date("Y-m-d", strtotime($r->date_of_joining)));
            if ($r->date_of_leaving != '' || $r->date_of_leaving != null) {
                $date2 = new DateTime(date("Y-m-d", strtotime($r->date_of_leaving)));
            } else {
            ...
            if ($diff->y > 0) {
                ...
                $work_duration = $diff->m." months, ".$diff->d." days";
            } else {
                $work_duration = $diff->d." days";
            }
    ...
    public function training_list()
    {
        ...
        // get training type
                    ...
                    // status
                    if ($r->training_status==0): $status = $this->lang->line('xin_pending'); elseif ($r->training_status==1): $status = $this->lang->line('xin_started'); elseif ($r->training_status==2): $status = $this->lang->line('xin_completed'); else: $status = $this->lang->line('xin_terminated');
                    endif;
    ...
    public function employee_date_wise_list()
    {
        ...
        $employees=array();
        if ($employee_id!=0) {
            $employees = $this->Xin_model->read_user_info($employee_id);
        } else {
            $employees = $this->Xin_model->get_company_employees($company_id)->result();
        }
        $data = array();
        foreach ($employees as $employee) {
            ...
            foreach ($date_range as $date) {
                ...
                // office shift
                $office_shift = $this->Timesheet_model->read_office_shift_information($employee->office_shift_id);
                ...
                $attendance_status = '';
                $check = $this->Timesheet_model->attendance_first_in_check($employee->user_id, $attendance_date);
                if ($check->num_rows() > 0) {
                    // check clock in time
                    $attendance = $this->Timesheet_model->attendance_first_in($employee->user_id, $attendance_date);
                    // clock in
                    $clock_in = new DateTime($attendance[0]->clock_in);
                    $clock_in2 = $clock_in->format('h:i a');
                    $clkInIp = $clock_in2.'<br><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-ipaddress="'.$attendance[0]->clock_in_ip_address.'" data-uid="'.$employee->user_id.'" data-att_type="clock_in" data-start_date="'.$attendance_date.'"><i class="ft-map-pin"></i> '.$this->lang->line('xin_attend_clkin_ip').'</button>';
                    ...
                    // total hours work/ed
                    $total_hrs = $this->Timesheet_model->total_hours_worked_attendance($employee->user_id, $attendance_date);
                    $hrs_old_int1 = 0;
                    $Total = '';
                    $Trest = '';
                    $hrs_old_seconds = 0;
                    ...
                    // total rest >
                    $total_rest = $this->Timesheet_model->total_rest_attendance($employee->user_id, $attendance_date);
                    ...
                    // get leave/employee
                    $leave_date_chck = $this->Timesheet_model->leave_date_check($employee->user_id, $attendance_date);
                    $leave_arr = array();
                    if ($leave_date_chck->num_rows() == 1) {
                        $leave_date = $this->Timesheet_model->leave_date($employee->user_id, $attendance_date);
                ...
                $check_out = $this->Timesheet_model->attendance_first_out_check($employee->user_id, $attendance_date);
                if ($check_out->num_rows() == 1) {
                    /* early time */
                    $early_time =  new DateTime($out_time.' '.$attendance_date);
                    // check clock in time
                    $first_out = $this->Timesheet_model->attendance_first_out($employee->user_id, $attendance_date);
                    // clock out
                    $clock_out = new DateTime($first_out[0]->clock_out);

                    if ($first_out[0]->clock_out!='') {
                        $clock_out2 = $clock_out->format('h:i a');
                        $clkOutIp = $clock_out2.'<br><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-ipaddress="'.$attendance[0]->clock_out_ip_address.'" data-uid="'.$employee->user_id.'" data-att_type="clock_out" data-start_date="'.$attendance_date.'"><i class="ft-map-pin"></i> '.$this->lang->line('xin_attend_clkout_ip').'</button>';
                        // early leaving
