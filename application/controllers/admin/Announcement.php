...
class Announcement extends MY_Controller {
	...
	public function add_announcement() {
		...
		$module_attributes = $this->Custom_fields_model->announcements_hrms_module_attributes();
		...
		if ($iresult) {
			...
			$id = $iresult;
			// notificaions
			$dep_employees = $this->Xin_model->get_department_employees_announcement($this->input->post('department_id'),$this->input->post('location_id'));

                        $to_email = array();
                        foreach($dep_employees as $demployees){
                                if($demployees->email!=null || !empty($demployees->email)){
                                    $to_email[] = $demployees->email;
                                }
				$nticket_data = array(
				...
				$this->Xin_model->add_notifications($nticket_data);
                            ...
                            //get company info
                            $cinfo = $this->Xin_model->read_branch_setting_info($this->input->post('company_id'));
                            ...
                            <img src="' . $logo . '" title="' . $cinfo[0]->name . '"><br>' . str_replace(array("{var site_name}","{var summary}","{var description}"), array($cinfo[0]->name,$this->input->post('summary'),$qt_description), htmlspecialchars_decode(stripslashes($template[0]->message))) . '</div>';

                            $to_mail=array_chunk($to_email,10);
                            if (!empty($to_mail)) {
                                foreach ($to_mail as $mail){
                                $emails=implode(', ', $mail);
                               
                                hrms_mail($cinfo1[0]->email, $cinfo[0]->name,$emails,$subject,$message);
