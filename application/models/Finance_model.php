...
class Finance_model extends CI_Model {
	...
	public function get_expense() {
	  return $this->db->query("SELECT * from xin_finance_transaction where transaction_type = 'expense'");
        ...
        //tax chalan
        public function get_tax_chalan() {
            return $this->db->get("xin_tax_chalans");
	}
	...
	public function get_all_rinning_project()
	{
	  $query = $this->db->query("SELECT * from xin_projects WHERE status =1 ");
  	  return $query->result();
	}
	// get all table rows 
	...
	public function add_transactions($data){
        ...
        public function add_tax_chalan($data){
		$this->db->insert('xin_tax_chalans', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
        ...
        public function delete_tax_chalan($id){
		$this->db->where('id', $id);
		$this->db->delete('xin_tax_chalans');
		
	}
        public function read_tax_chalan($id) {
	
		$sql = 'SELECT * FROM xin_tax_chalans WHERE id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
        ...
        public function update_tax_chalan($data, $id){
		$this->db->where('id', $id);
		if( $this->db->update('xin_tax_chalans',$data)) {
			return true;
		} else {
			return false;
		}		
	...
	public function get_income_expense_search($start_date,$end_date){
        ...
        // expense > ebl_connection	
	public function get_ebl_connection_search($start_date,$end_date,$type_id,$company_id){
		if($type_id=='none') {
			
			$sql = 'SELECT * FROM `xin_finance_transaction` where transaction_type = "eexpense" and payee_option = 2 and DATE(transaction_date) BETWEEN ? AND ?';
			$binds = array($start_date,$end_date);
			$query = $this->db->query($sql, $binds);
			return $query;
			...
			$sql = 'SELECT * FROM `xin_finance_transaction` where transaction_type = "expense" and payee_option = 2 and DATE(transaction_date) BETWEEN ? AND ?';
			$binds = array($start_date,$end_date);
			$query = $this->db->query($sql, $binds);
			return $query;
			...
			$sql = 'SELECT * FROM `xin_finance_transaction` where transaction_type = "expense" and payee_option = 2 and company_id = ? and DATE(transaction_date) BETWEEN ? AND ?';
			$binds = array($company_id,$start_date,$end_date);
			$query = $this->db->query($sql, $binds);
			return $query;
		} else if($company_id!=0 && $type_id!=0) {
			
			$sql = 'SELECT * FROM `xin_finance_transaction` where transaction_type = "expense" and payee_option = 2 and company_id = ? and transaction_cat_id = ? and DATE(transaction_date) BETWEEN ? AND ?';
			$binds = array($company_id,$type_id,$start_date,$end_date);
			$query = $this->db->query($sql, $binds);
			return $query;
		}
