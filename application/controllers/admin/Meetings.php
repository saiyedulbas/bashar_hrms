...
class Meetings extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function calendar() {
		...
		$data['title'] = $this->lang->line('xin_hrms_meetings_calendar');
		$data['breadcrumbs'] = $this->lang->line('xin_hrms_meetings_calendar');
	...
	public function add_meeting() {
		...
		if($Return['error']!=''){
       		$this->output($Return);
                ...
                if(in_array("all", $this->input->post('employee_id'))){
                    $employes=$this->Employees_model->get_employees();
                    foreach($employes->result() as $r) {
                         $assigned_ids[]=$r->user_id;
                    }
                   $assigned_ids= implode(',',$assigned_ids);
