...
class Application_update extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function upload_sql_only() {
		...
		if($id=='is_ajax') {
			...
			$db = array('default' => array());
					...
					$remote_file_url = 'http://www.hrms.com/hrms_updates/abc.zip';
					
					// get the absolute path to $file
					$local_file = base_url();//'http://localhost/hrms/uploads/csv/';//pathinfo(realpath($file), PATHINFO_DIRNAME);
					
			
					$full_path = 'http://localhost/hrms/abc.zip';
			...
			$sql = file_get_contents('http://www.hrms.com/hrms_updates/update_v1.1.7_to_v1.1.8test.sql');
	...
	public function file_upload()
	 {
		 $local_file = 'http://localhost/hrms/uploads/csv/';
			...
			if($_FILES['file_zip']['size'] == 0) {
			 $Return['error'] = $this->lang->line('xin_hr_error_zip_installation');
				...
				if(in_array($ext,$allowed)){
					$zip = new ZipArchive;
					// assuming file.zip is in the same directory as the executing script.
					$remote_file_url = 'http://localhost/hrms/ziptest.zip';
					
					// get the absolute path to $file
					$local_file = 'http://localhost/hrms/uploads/csv/';//pathinfo(realpath($file), PATHINFO_DIRNAME);
					...
					/*$zip = new ZipArchive;
					$res = $zip->open('http://localhost/hrms/uploads/csv/ziptest.zip');
					if ($res === TRUE) {
					  // extract it to the path we determined above
					  $zip->extractTo('http://localhost/hrms/uploads/csv/');
					  $zip->close();
