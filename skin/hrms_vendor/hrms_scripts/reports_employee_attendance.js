   ...
   var xin_table = $('#xin_table').dataTable({
	...
	$("#attendance_datewise_report").submit(function(e){
		...
		var user_id = $('#employee_id').val();
                var comapny_id = $('#aj_company').val();
		var xin_table2 = $('#xin_table').dataTable({
			"bDestroy": true,
			"ajax": {
				url: site_url + "reports/employee_date_wise_list/?start_date=" + start_date + "&end_date=" + end_date + "&user_id=" + user_id + "&company_id=" + comapny_id,
                                type: 'GET'
			},
			dom: 'lBfrtip',
			"buttons": ['csv', 'excel', 'pdf', 'print'], // colvis > if needed
