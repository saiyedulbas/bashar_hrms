<?php
    ...
    class Timesheet_model extends CI_Model
    {
        ...
        {
            $sql = 'SELECT * FROM xin_leave_applications where company_id IN ? order by leave_id desc';
        ...
        {
            $sql = 'SELECT * FROM xin_leave_applications WHERE company_id = ? and employee_id = ? order by leave_id desc';
        ...
        {
            $sql = 'SELECT * FROM xin_leave_applications WHERE company_id = ? and employee_id = ? and status = ? order by leave_id desc';
        ...
        {
            $sql = 'SELECT * FROM xin_leave_applications WHERE company_id = ? and status = ? order by leave_id desc';
        ...
        {
            $sql = 'SELECT * from xin_leave_applications where employee_id = ? and from_date = ? and to_date = ? limit 1';
        ...
        public function delete_gurbage_data()
        {
            $this->db->where('employee_id', 0);
            $this->db->or_where('clock_in', '');
        ...
        public function update_earn_leave_record($id,$data)
        {
             $this->db->where('id', $id);
        ...
        public function get_all_work_time()
        {
            $today_date = date('Y-m-d');
            $session = $this->session->userdata('username');
            $sql = 'SELECT * FROM xin_attendance_time where `employee_id` = ? and `attendance_date` = ? and `total_work`!=?    order by time_attendance_id desc limit 1';
            $binds = array($session['user_id'],$today_date,'');
            $query = $this->db->query($sql, $binds);
        ...
        {
            $sql = 'SELECT * FROM xin_leave_applications WHERE department_id = ? and YEAR(to_date) = YEAR(CURDATE())';
        ...
        //$sql = 'SELECT * FROM xin_leave_applications WHERE employee_id = ? and leave_type_id = ? and status = ? and created_at >= DATE_SUB(NOW(),INTERVAL 1 YEAR)';
            $sql = 'SELECT * FROM xin_leave_applications WHERE employee_id = ? and leave_type_id = ? and YEAR(to_date) = YEAR(CURDATE()) and status = ?';
            $binds = array($employee_id,$leave_type_id,2);
            $query = $this->db->query($sql, $binds);

            return $query->num_rows();
        }
        public function employee_earn_leaves($employee_id) {
            $sql = 'SELECT leave_balance as leave_balance, leave_taken as leave_taken FROM xin_employee_earn_leave WHERE employee_id = ? AND status=? order by id desc';
            $binds = array($employee_id,0);
            $query = $this->db->query($sql, $binds);

            return $query->row();

        }
        public function employee_curent_earn_leaves($employee_id) {
            $sql = 'SELECT id,leave_balance,leave_taken FROM xin_employee_earn_leave WHERE employee_id = ? AND status=? order by id desc limit 1';
            $binds = array($employee_id,0);
            $query = $this->db->query($sql, $binds);
        ...
        // Get employee monthly food allowance amount
        public function employee_monthly_food_allowance_calculator($emp_id, $month, $year)
        {
            $sql = "SELECT * FROM (SELECT *, MONTH(attendance_date) month_val, YEAR(attendance_date) year_val FROM xin_attendance_time) as employee
								WHERE  employee.month_val = ? and employee.year_val = ? and
								employee.time_attendance_id in (SELECT MAX(time_attendance_id) FROM xin_attendance_time
								where employee_id = ? GROUP BY attendance_date)";
            $binds = array($month, $year, $emp_id);
            $query = $this->db->query($sql, $binds);
            $count = 0;
            if ($query->num_rows() > 0) {
                foreach ($query->result() as $row) {
                    if (substr($row->total_work, 0, 2) > 6) {
                        $count++;
                    }
                }

                $this->db->where('user_id', $emp_id);
                $query1 = $this->db->get("xin_employees");
                $employee = $query1->result();
                return $employee[0]->daily_food_allowance * $count;
            }
            return 0;
        }
    }
