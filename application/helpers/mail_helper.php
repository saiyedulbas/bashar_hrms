<?php
//Process String
if( !function_exists('hrms_mail') ){

	function hrms_mail($from,$from_name,$to,$subject,$body){
...
// company info
if( !function_exists('hrms_company_name') ){

 function hrms_company_name(){
  $CI=& get_instance();
  $query =  $CI->db->query("SELECT company_name FROM xin_company_info")->row()->company_name;
  return $query;
 }
}
if( !function_exists('hrms_company_email') ){

 function hrms_company_email(){
  $CI=& get_instance();
